...
namespace WebAdmin.Client.Services.Services
{
    public class HttpOrderService : IOrderService
    {
        ...
        public async Task<PagedList<OrderDetail>> GetOrdersAsync(string userId, string status = null, DateTime? fromDate = null, DateTime? toDate = null, int pageNumber = 1, int pageSize = 10)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                var result = await response.Content.ReadFromJsonAsync<PagedList<OrderDetail>>();
                return result;
            }
            else
            {
